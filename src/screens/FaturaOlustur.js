...
const getCategoryDisplayName = (category) => {
  const map = {
    Electricity: 'Elektrik', Water: 'Su', Internet: 'İnternet',
    Rent: 'Kira', Gas: 'Doğalgaz', Market: 'Market', Food: 'Yemek', Other: 'Diğer',
...
const UtilityBillCreateScreen = ({ route, navigation }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      if (houseId) fetchMembers();
    }, [houseId])
  );

  useEffect(() => {
    const type = BILL_TYPES.find(t => t.key === billType);
    if (!type) return;
    if (type.isFixed === true) { setIsRecurring(true);  showToast(`${type.label} - ${type.description}`, 'info'); }
    if (type.isFixed === false){ setIsRecurring(false); showToast(`${type.label} - ${type.description}`, 'info'); }
    if (type.isFixed === null) { setIsRecurring(false); }
  }, [billType]);
  ...
  const fetchBillData = async () => {
    try {
      ...
      setBillType(typeof bill.category === 'number'
...
//////////////////////
  ...
  return (
    ...
    >
      <LinearGradient colors={[theme.colors.primary[600], theme.colors.primary[500]]} style={styles.container}>
        <Animated.View style={[styles.content, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      selected && styles.selectedBillTypeButton,
                      type.isFixed === true && styles.fixedBillTypeButton,
                      type.isFixed === false && styles.variableBillTypeButton
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        ...
                        selected && styles.selectedBillTypeButtonText,
                        type.isFixed === true && styles.fixedBillTypeButtonText,
                        type.isFixed === false && styles.variableBillTypeButtonText
                      ]}
                    >
                      {type.label}
                    </Text>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.pickerContainer}>
              {members.map((m) => {
                const selected = Number(m.userId) === Number(responsibleUserId);
                return (
                  <TouchableOpacity
                    key={m.userId}
                    style={[styles.memberButton, selected && styles.selectedMemberButton]}
                    onPress={() => setResponsibleUserId(m.userId)}
                    activeOpacity={0.8}
                  >
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            {loading ? <ActivityIndicator color={theme.colors.text.onPrimary} size="small" /> :
